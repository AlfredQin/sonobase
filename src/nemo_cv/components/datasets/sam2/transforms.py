...
from .data_utils import VideoDatapoint
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    for obj in datapoint.frames[index].objects:
        if obj.segment is not None:
            # Masks are uint8 {0,1}; NEAREST avoids bilinear interpolation
            # eroding/dilating boundaries at non-aligned target sizes.
            # Image resize above stays bilinear by design.
            obj.segment = F.resize(
                obj.segment[None, None], size,
