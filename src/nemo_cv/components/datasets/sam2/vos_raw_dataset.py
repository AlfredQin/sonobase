...
from .vos_segment_loader import (
    JSONSegmentLoader,
...
class SaUSRawDataset(VOSRawDataset):
    """Dataset for ultrasound images in SA-1B annotation format.

    Unlike SA1BRawDataset, this class does NOT assume the filename format is sa_{int}.
    It uses the dataset index as video_id and keeps the full filename as video_name,
    allowing arbitrary naming conventions (e.g. AbdomenUS_RUS_test_001.jpg).
    """
    ...
    def __init__(
        ...
        if file_list_txt is not None:
            ...
            subset = os.listdir(self.img_folder)
            subset = [os.path.splitext(path)[0] for path in subset if path.endswith(self.image_ext)]

        if excluded_videos_list_txt is not None:
            with g_pathmgr.open(excluded_videos_list_txt, "r") as f:
                excluded_files = set(os.path.splitext(line.strip())[0] for line in f)
        else:
            excluded_files = set()

        self.video_names = sorted([name for name in subset if name not in excluded_files])
        logging.info(f"SaUSRawDataset: loaded {len(self.video_names)} images from {img_folder}")

    def get_video(self, idx):
        video_name = self.video_names[idx]
        video_frame_path = os.path.join(self.img_folder, video_name + self.image_ext)
        video_mask_path = os.path.join(self.gt_folder, video_name + ".json")
