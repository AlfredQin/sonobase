    ...
    def __init__(
        ...
        if self.with_cross_branch_interaction:
            for idx, (dim0, dim1, dim2) in enumerate(zip(branch0_dims, branch1_dims, branch2_dims)):
                ...
                elif interact_type == "deform_cross_attn":
                    self.interaction_blocks.append(
                        DeformCrossAttnTripleBranchFusion(
                            dim0,
                            dim1,
                            dim2,
                            num_heads0=num_heads[idx],
                            num_heads1=num_heads[idx],
                            num_heads2=num_heads[idx],
                            n_points=n_points,
    ...
    def forward(self, images: torch.Tensor) -> List[torch.Tensor]:
        ...
        for i, (indices0, indices1, indices2) in enumerate(zip(self.branch0_indices, self.branch1_indices, self.branch2_indices)):
            ...
            if self.merge_location == "before_interact":
                ...
                # fusion
                if self.with_cross_branch_interaction and i != self.return_indices[-1]:  # we dont need to do fusion for the last block
...
class MSDeformSelfAttnCrossBranchLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = MSDeformAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads, n_points=n_points)
...
class MSDeformSelfAttnCrossBranchBlock(nn.Module):
    def __init__(
        ...
        self.level_embed = nn.ParameterList([nn.Parameter(torch.randn(dim)) for _ in range(num_levels)])
...
class DeformCrossAttnLayer(nn.Module):
    """
    Cross branch features fusion block with deformable cross attention
    """
    def __init__(
            self,
            dim,
        ...
        self.query_norm = nn.LayerNorm(dim) if query_norm else nn.Identity()
        self.value_norm = nn.LayerNorm(dim)
        self.cross_attn = MSDeformAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads, n_points=n_points)
        ...
        self.ls_attn = LayerScale(dim=dim, init_values=init_values)
        ...
        if ffn == "mlp":
            self.ffn = MLP(dim, int(dim * expand_ratio), dim, num_layers=2, activation=nn.GELU)
        elif ffn == "conv_ffn":
            self.ffn = ConvFFN(in_features=dim, hidden_features=int(dim * expand_ratio), drop=0.)

    def forward(self, query, value, reference_points, spatial_shapes, level_start_index, pos=None):
        """
        ...
        attn_out = self.cross_attn(
            query=self.with_pos_embed(self.query_norm(query.flatten(1, 2)), pos),
            reference_points=reference_points,
            input_flatten=self.value_norm(value.flatten(1, 2)),
            input_spatial_shapes=spatial_shapes,
            input_level_start_index=level_start_index,
        )
        query = query + self.drop_path_attn(self.dropout_attn(self.ls_attn(attn_out.view(B, H, W, C))))
        if hasattr(self, "ffn"):
            query = query + self.drop_path_ffn(self.ls_ffn(self.ffn(self.ffn_norm(query))))
...
class BidirectionalInteractionUnit(nn.Module):
    def __init__(
        ...
        super().__init__()
        if dim0 != dim1:
            self.b1tob0_proj = nn.Linear(dim1, dim0)
            self.b0tob1_proj = nn.Linear(dim0, dim1)
        else:
            self.b1tob0_proj = nn.Identity()
            self.b0tob1_proj = nn.Identity()

        self.b0_cross_attn_b1 = DeformCrossAttnLayer(
            dim=dim0,
            num_heads=num_heads0,
            n_points=n_points,
            dropout_attn=dropout_attn,
            drop_path_attn=drop_path_attn,
            drop_path_ffn=drop_path_ffn,
        ...
        self.b1_cross_attn_b0 = DeformCrossAttnLayer(
            dim=dim1,
    ...
    def forward(self, b0, b1, deform_inputs0, deform_inputs1, pos0=None, pos1=None,):
        """
        @param multi_branch_features: List of B x H x W x C features
        @return:
        """
        b0_after_attn = self.b0_cross_attn_b1(
            query=b0,
            value=self.b1tob0_proj(b1),
            reference_points=deform_inputs0['reference_points'],
            spatial_shapes=deform_inputs0['spatial_shapes'],
            level_start_index=deform_inputs0['level_start_index'],
            pos=pos0,
        )
        b1_after_attn = self.b1_cross_attn_b0(
            query=b1,
            value=self.b0tob1_proj(b0),
            reference_points=deform_inputs1['reference_points'],
            spatial_shapes=deform_inputs1['spatial_shapes'],
            level_start_index=deform_inputs1['level_start_index'],
            pos=pos1,
        )

        return b0_after_attn, b1_after_attn


class DeformCrossAttnTripleBranchFusion(nn.Module):
    def __init__(
            self,
            dim0,
            dim1,
        ...
        super().__init__()
        self.branch0_interact_branch1 = BidirectionalInteractionUnit(
                dim0=dim0,
                dim1=dim1,
                num_heads0=num_heads0,
        ...
        self.branch1_interact_branch2 = BidirectionalInteractionUnit(
                dim0=dim1,
                dim1=dim2,
                num_heads0=num_heads1,
                num_heads1=num_heads2,
                n_points=n_points,
                dropout_attn=dropout_attn,
                drop_path_attn=drop_path_attn,
                drop_path_ffn=drop_path_ffn,
        ...
        if with_pos:
            self.pos_encoding0 = PositionEmbeddingSine(num_pos_feats=dim0 // 2, normalize=True)
            self.pos_encoding1 = PositionEmbeddingSine(num_pos_feats=dim1 // 2, normalize=True)
            self.pos_encoding2 = PositionEmbeddingSine(num_pos_feats=dim2 // 2, normalize=True)
    ...
    def forward(self, f0, f1, f2):
        ...
        @return:
        """
        deform_inputs0 = self.get_deform_attn_inputs(f0, f1)
        deform_inputs1 = self.get_deform_attn_inputs(f1, f0)
        deform_inputs2 = self.get_deform_attn_inputs(f1, f2)
        deform_inputs3 = self.get_deform_attn_inputs(f2, f1)
        pos0, pos1, pos2 = None, None, None
        if self.with_pos:
            pos0 = self.pos_encoding0(f0.permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)
            pos1 = self.pos_encoding1(f1.permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)
            pos2 = self.pos_encoding2(f2.permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)

        if self.order == "bottom-up":
            f1, f2 = self.branch1_interact_branch2(f1, f2, deform_inputs2, deform_inputs3, pos1, pos2)
            f0, f1 = self.branch0_interact_branch1(f0, f1, deform_inputs0, deform_inputs1, pos0, pos1)
        else:
            f0, f1 = self.branch0_interact_branch1(f0, f1, deform_inputs0, deform_inputs1, pos0, pos1)
            f1, f2 = self.branch1_interact_branch2(f1, f2, deform_inputs2, deform_inputs3, pos1, pos2)
    ...
    def get_deform_attn_inputs(self, query, value):
        device = query.device
        spatial_shapes = [value.shape[1:3]]
        reference_points = get_reference_points([query.shape[1:3]], device)
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=device)
        level_start_index = torch.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))

        deform_inputs = {"reference_points": reference_points,
...
class DeformCrossAttnDoubleBranchFusion(nn.Module):
    def __init__(
            self,
            dim0,
            dim1,
        ...
        super().__init__()
        self.branch0_interact_branch1 = nn.ModuleList([
            BidirectionalInteractionUnit(
                dim0=dim0,
                dim1=dim1,
                num_heads0=num_heads0,
                num_heads1=num_heads1,
    ...
    def forward(self, f0, f1):
        ...
        for layer in self.branch0_interact_branch1:
            f0, f1 = layer(f0, f1, deform_inputs0, deform_inputs1, pos0, pos1)
...
class MSDWConv(nn.Module):
    ...
    def forward(self, x, spatial_shapes):
        """
        :param x: concat multi-scale features
        :param H: the height of top level features, which has lowest resolution
        :param W: the width of top level features, which has lowest resolution
        :return:
        """
        B, N, C = x.shape
        x = torch.split(x, [shape[0] * shape[1] for shape in spatial_shapes], dim=1)
        x = [feat.view(B, *shape, -1).permute(0, 3, 1, 2) for feat, shape in zip(x, spatial_shapes)]
        x = [self.dwconv(feat).flatten(2).permute(0, 2, 1) for feat in x]
        x = torch.cat(x, dim=1)

        return x


class MSDWConvFFN(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = nn.Linear(in_features, hidden_features)
        self.dw_conv = MSDWConv(dim=hidden_features)
        self.act = act_layer()
        self.fc2 = nn.Linear(hidden_features, out_features)
        self.drop = nn.Dropout(drop)
    ...
    def forward(self, x, spatial_shapes):
        x = self.fc1(x)
        x = self.dw_conv(x, spatial_shapes)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
