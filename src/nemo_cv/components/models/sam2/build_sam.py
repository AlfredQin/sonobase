...
def _get_sam2_configs_dir():
    """Get the absolute path to the SAM2 configs directory."""
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "../sonobase/configs")


def _compose_hydra_config(config_file, overrides=None):
    """
    Compose a Hydra config, handling config path initialization.
    
    Args:
        config_file: Config file path (relative to configs dir or absolute path)
    ...
    # If config_file is an absolute path, use OmegaConf directly
    ...
    # Handle relative paths - normalize the config file path
    # Strip 'configs/' prefix if present, and remove .yaml extension for Hydra
    config_name = config_file
    if config_name.startswith("configs/"):
        config_name = config_name[len("configs/"):]
    
    # Get configs directory
    config_dir = _get_sam2_configs_dir()
