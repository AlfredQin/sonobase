#!/bin/bash
# Stage 2 — T1.1: significance tests + BH-FDR across A1/A2/A4/C2 (box-prompt run).
# Requires the named per-sample CSVs to exist from prior box-prompt Stage-2 runs.
set -e
source "$(dirname "$(readlink -f "$0")")/_paths.sh"

OUT=${ANALYSIS_OUT_ROOT}/t1_1_significance/box
...
if [ -z "$INPUTS" ]; then
  echo "T1.1 (box): no per-sample CSVs found. Run A1/A2/A4/C2 box-prompt scripts first."
  exit 1
fi
