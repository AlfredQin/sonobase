#!/bin/bash
# Stage 2 — B3: Porcine spinal cord per-class mIoU.
set -e
source "$(dirname "$(readlink -f "$0")")/_paths.sh"

OUT=${ANALYSIS_OUT_ROOT}/b3_porcine_spinal/PorcineSpinalCord_point_0corr
uv run python -m nemo_cv.recipes.analysis.b3_porcine_spinal \
  --runs sam2_no_ft=${PRED_ROOT}/sam2_no_ft_PorcineSpinalCord_point_0corr \
         medsam2=${PRED_ROOT}/medsam2_PorcineSpinalCord_point_0corr \
         sonobase=${PRED_ROOT}/sonobase_PorcineSpinalCord_point_0corr \
  --output-dir ${OUT}
