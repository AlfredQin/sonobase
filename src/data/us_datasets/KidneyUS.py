"""KidneyUS dataset preprocessing.

Dataset: Kidney ultrasound segmentation dataset
Source: https://github.com/rsingla92/kidneyUS

Data format:
- kidneyUS_images_14_june_2022.zip: Ultrasound images (PNG)
- labels/reviewed_masks_1/capsule/{id}.png: Kidney capsule masks
...
CATEGORIES = [
    {"supercategory": "organ", "id": 1, "name": "kidney"},
]
...
def collect_samples(data_dir: Path) -> List[Dict]:
    ...
    # Find all images (may be in nested image archive or directly)
    img_files = {}
    for png_file in data_dir.rglob("*.png"):
        # Skip mask files
        if "mask" in str(png_file).lower() or "label" in str(png_file).lower():
            continue
        img_files[png_file.stem] = png_file

    # Find mask directories (reviewed_masks_1 or reviewed_masks_2)
    mask_dirs = []
    for candidate in data_dir.rglob("capsule"):
        if candidate.is_dir():
    ...
    # Use first found mask directory
...
def main():
    ...
    with temp_context as temp_dir:
        if use_temp_dir:
            ...
            if (data_dir / "kidneyUS").exists():
                data_dir = data_dir / "kidneyUS"
            
            # Extract nested image archive if present
            nested_zip = data_dir / "kidneyUS_images_14_june_2022.zip"
            if nested_zip.exists():
                logger.info("Extracting nested image archive")
                shutil.unpack_archive(str(nested_zip), data_dir / "images")
        else:
            data_dir = data_path
