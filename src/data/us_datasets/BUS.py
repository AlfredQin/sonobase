"""BUS (Breast Ultrasound) dataset preprocessing.

Dataset: Classic breast ultrasound dataset
Source: Request @ http://www2.docm.mmu.ac.uk/STAFF/M.Yap/dataset.php

Data format:
- original/{id}.png: Ultrasound image
- GT/{id}.png: Ground truth mask
...
def parse_args():
    ...
    parser.add_argument(
        "--path",
        type=str,
        default="/mnt/data/Dataset/UltraSound/Raw/BUS.zip",
...
def collect_samples(data_dir: Path) -> List[Dict]:
    ...
    if (data_dir / "BUS").exists():
        data_dir = data_dir / "BUS"
    
    img_dir = data_dir / "original"
    mask_dir = data_dir / "GT"
